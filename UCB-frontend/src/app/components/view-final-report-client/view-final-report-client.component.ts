...
@Component({
  selector: 'app-view-final-report-client',
  templateUrl: './view-final-report-client.component.html',
  styleUrl: './view-final-report-client.component.css'
})
export class ViewFinalReportClientComponent {
  ...
  constructor(private reportService:ReportService, private router:Router, private route: ActivatedRoute, private clientService:ClientService, private companyService:CompanyService){
    ...
    const reportfinalId = localStorage.getItem('report-finalId');
    ...
    if (reportfinalId) {
      const secretKey = 'Dr34ml4b';
      // Descifrar el valor cifrado utilizando AES
      const bytes = CryptoJS.AES.decrypt(reportfinalId, secretKey);
      const decryptedAuditIdString = bytes.toString(CryptoJS.enc.Utf8);
  
      // Convertir la cadena de texto descifrada de vuelta a número
      this.reportFinalId  = parseInt(decryptedAuditIdString, 10);
    }
    if (id) {
      const secretKey = 'Dr34ml4b';
      // Descifrar el valor cifrado utilizando AES
    ...
    this.clientService.getCustomerById(this.id).subscribe((data: any) => {
        //Obtener informacion del cliente 
        this.companyService.getCompanyById(data.data.companyId).subscribe((data: any) => {
